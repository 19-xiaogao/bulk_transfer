...
contract BatchTransfer is Refundable {
    ...
    // vip 账户折扣, 优惠百分比 [0-100]
    mapping(address => uint256) private _vips;

    constructor(
        address owner_,
        uint256 baseFee, // 基础手续费
        uint256 unitFee, // 每笔交易的手续费
        uint256 refBonus // 优惠比例
    ) Refundable(owner_) {
        _baseFee = baseFee;
        _unitFee = unitFee;
    ...
    function sendETH(
        ...
        address payable referrer // 推荐
    ) public payable {
        uint256 txCount = payees.length;
        require(txCount == amounts.length, "Params not match");

        uint256 remain = msg.value; // gas费用...
        uint256 fee = calcFee(msg.sender, txCount); // 计算这个地址所需要的gas飞
    ...
    function sendToken(
        ...
        for (uint256 i = 0; i < txCount; i++) {
            // 必须用 safeTransferFrom, 否则 usdt 的 onlyPayloadSize 会使合约调用的交易失败
